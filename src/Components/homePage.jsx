...
function HomePage() {
  ...
  const fetchSorah = async () => {
    try {
      const response = await fetch("https://corsproxy.io/?" + encodeURIComponent("https://alquran-api.pages.dev/api/quran?lang=en"));
  ...
  const fetchAyahs = async (surahId) => {
    try {
      ...
      const response = await fetch("https://corsproxy.io/?" + encodeURIComponent(`https://alquran-api.pages.dev/api/quran/surah/${surahId}?lang=ar`));
      const data = await response.json();
      setAyahs(data);
      setCurrentWordIndex(0);
